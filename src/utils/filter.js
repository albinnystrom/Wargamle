...
function keyFilter(key, units) {
    ...
    if (key == "year" || key == "speed") {
        ...
        const lwr =
            summaryVals[key][0] != "?" ? Number(summaryVals[key][0]) : -1;
        const upr =
            summaryVals[key][1] != "?" ? Number(summaryVals[key][1]) : 10000;
        return units.filter(
            (u) => Number(u[key]) >= lwr && Number(u[key]) <= upr
        );
    }
    return units;
}
...
export function filterUnits(units) {
    ...
    units = units.filter(
        (u) =>
            !(
                sharedObjects.guessedUnits.includes(u.name) &&
                sharedObjects.guessedUnits.includes(u.country)
            )
    );
    return units;
}
